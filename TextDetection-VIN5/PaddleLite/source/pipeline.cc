...
cv::Mat Pipeline::Process(cv::Mat img,std::vector<std::string> &res_txt,std::vector<float> &res_txt_score) {
  //  Timer tic;
  //  tic.start();
  ...
  res_txt.resize(rec_text.size());
  res_txt_score.resize(rec_text_score.size());
  for (int i = 0; i < rec_text.size(); i++) {
    //    std::cout << i << "\t" << rec_text[i] << "\t" << rec_text_score[i]
    //              << std::endl;
    res_txt[i] = rec_text[i];
    res_txt_score[i] = rec_text_score[i];
  }
  return img_vis;
}
